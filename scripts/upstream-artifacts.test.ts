...
const ADDED_FIXTURE = [
  ...
  {
    path: 'Anwenderkataloge/WLAN/WLAN-catalog.json',
  ...
  {
    path: 'Implementierungsbeschreibungen/Komponenten/GA-Lotse_Grundmodul/GA-Lotse_Grundmodul-component_definition.json',
    headSha: 'ebce4b9886ecda020911279e48f6e94f796b9f51',
  },
  {
    path: 'Implementierungsbeschreibungen/Komponenten/Lieferkettensicherheit/Lieferkettensicherheit-component_definition.json',
    headSha: '1018a02d9850eccad3146c478eaad62c1bf79e9d',
  },
  {
    path: 'Mappings/ISO-27001-zu-GSpp/ISO27001-AnnexA-catalog.json',
    headSha: 'b42bf1338bee3673db32b85306125330001d5ad9',
  },
  {
    path: 'Mappings/ISO-27001-zu-GSpp/ISO27001-AnnexA-to-GS++-mapping.json',
    headSha: '7404229d890158f1cf9ce46e029cffb961673106',
  },
  {
    path: 'Mappings/IT-GS2023-zu-GSpp/ITGS-to-GS++-mapping.json',
    headSha: 'c4c869f40d2afee1f2efb337c7f3232fed1a1292',
  },
  {
    path: 'Quellkataloge/Lieferkettensicherheit/Lieferkettensicherheit-profile.json',
    headSha: 'e00688774ae550f8a3cd0d9d57b25973fa8b334f',
  },
  {
    path: 'Quellkataloge/WLAN/WLAN-profile.json',
...
describe('upstream snapshot tree delta', () => {
  it('reports all 17 official d6153cbb to 12abb438 paths and classifications', () => {
    const baseTree = tree(
    ...
    expect(delta.map(({ status, path }) => ({ status, path }))).toEqual([
      ...
      { status: 'added', path: 'Anwenderkataloge/WLAN/WLAN-catalog.json' },
      { status: 'added', path: 'Dokumentation/namespaces/security_targets_levels.csv' },
      {
        status: 'added',
        path: 'Implementierungsbeschreibungen/Komponenten/GA-Lotse_Grundmodul/GA-Lotse_Grundmodul-component_definition.json',
      },
      ...
      { status: 'added', path: 'Quellkataloge/WLAN/WLAN-profile.json' },
    ]);
    expect(
      delta.find(
        (entry) => entry.path === 'Dokumentation/namespaces/security_targets_levels.csv',
      ),
    ).toEqual({
      status: 'added',
      path: 'Dokumentation/namespaces/security_targets_levels.csv',
...
describe('upstream manifest v2', () => {
  ...
  it('builds path-sorted entries and a canonical v2 signature', () => {
    ...
    expect(manifest.files.map((file) => file.path)).toEqual([
      ...
      'Quellkataloge/WLAN/WLAN-profile.json',
    ]);
