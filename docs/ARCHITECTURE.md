...
Die Pull-Request-Prüfung liegt in zwei Workflows mit unterschiedlichen Ereignislisten. `.github/workflows/ci.yml` führt `documentation-contract` und `catalog-sync-guard`; beide urteilen über die Pull-Request-Metadaten und abonnieren deshalb zusätzlich `edited`. `.github/workflows/validate.yml` führt `validate` (Schema-Verifikation, Node-Versionsquelle, Scope-Entscheider, Lint vor dem ersten Netzschritt, Katalog-Fetch, Profilauflösung, go-oscal-Lauf, Tests mit Coverage, Playwright-Cache mit Browser-Tests, Build und als letzte Schritte desselben Jobs den zizmor-Audit) und abonniert `edited` nicht.

Ohne abonniertes `edited` entsteht bei einer Titel- oder Body-Bearbeitung kein neuer Lauf und damit kein neuer Check-Run; das Ergebnis des letzten Code-Laufs bleibt stehen. Ein per Bedingung übersprungener Job meldete dagegen den Status `Success` mit neuem Check-Run gleichen Namens und ersetzte das reale Ergebnis — deshalb läuft die Trennung über zwei Dateien statt über ein Job-`if`. Step-Skips innerhalb des einen Jobs erzeugen keine neuen Check-Runs; der Pflichtcheck bleibt genau dieser Lauf.

Der Scope-Entscheider (`scripts/ci-scope.mjs`) schreibt genau einen Wert nach `GITHUB_OUTPUT` (`full`, `docs_only` oder `manifest_only`). Profilauflösung und go-oscal entfallen bei `docs_only` (Upstream-Bytes gegen Manifest + Register ändern sich nicht), Browser-Cache, Chromium-Installation, Browser-Tests, Egress-Nachweis und Build laufen nur bei `full`; Fetch, Coverage, Lint, Schema-, Policy-, Versions- und zizmor-Gates laufen immer. Jeder Fetch-/Diff-Fehler fällt fail-closed auf `full` zurück.
