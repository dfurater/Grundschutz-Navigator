...
Die Pull-Request-Prüfung liegt in zwei Workflows mit unterschiedlichen Ereignislisten. `.github/workflows/ci.yml` führt `documentation-contract` und `catalog-sync-guard`; beide urteilen über die Pull-Request-Metadaten und abonnieren deshalb zusätzlich `edited`. `.github/workflows/validate.yml` führt `validate` (Schema-Verifikation, Node-Versionsquelle, Lint vor dem ersten Netzschritt, Katalog-Fetch, Profilauflösung, go-oscal-Lauf, Tests mit Coverage, Playwright-Cache mit Browser-Tests, Build und als letzte Schritte desselben Jobs den zizmor-Audit) und abonniert `edited` nicht.
