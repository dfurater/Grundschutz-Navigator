...
In der Control-Detailansicht öffnet der unterstrichene Gefährdungsbegriff die Vokabelkarte. Die Karte zeigt die Kennung in ihrer Kopfzeile („Elementare Gefährdung: G 0.18“) vor der Definition und blendet über `hiddenColumns` die dort redundante Spalte `Begriff` aus (`src/features/catalog/ControlSecurityContext.tsx`, `src/features/vocabularies/VocabularyEntryCard.tsx`). Ohne auflösbaren Begriff bleibt die Kennung direkt in der Merkmale-Liste sichtbar. Dasselbe kontextbezogene Ausblenden der Spalte `Begriff` gilt für das Praktik-Vokabular im Breadcrumb: Sie wird nur ausgeblendet, wenn der Wert exakt dem angezeigten Praktik-Namen entspricht (`src/features/catalog/ControlTaxonomyBreadcrumb.tsx`).

Die Kontrollansicht erklärt aufgelöste Vokabularbegriffe direkt an ihrem Vorkommen mit gepunkteter Unterstreichung und aufklappbarer Karte. Im Block „Anforderung“ stehen Modalverb, Sicherheitsniveau und Aufwand als Badges in der ersten Zeile; ein Link „Legende“ erläutert die vorkommenden Stufen. Der Anforderungssatz markiert darin erkannte Praktik-, Modalverb-, Handlungswort-, Ergebnis- und Präzisierungsabschnitte. Ein Parameter ohne gesetzten Wert erhält am Platzhalter eine per Hover oder Antippen erreichbare Erklärung, dass der Wert im eigenen Profil oder im Implementierungsplan (SSP) festgelegt wird. Unter dem Satz folgen Dokumentation und die beschriftete Gruppe „Zielobjekte“. In „Schutzziele und Gefährdungen“ stehen Schutzziele mit Relevanz in einem Raster und Gefährdungen als Aufzählung, in „Einordnung“ die beschriftete Gruppe „Tags“ und gegebenenfalls die WLAN-Taxonomie. Die Begriffe dort haben kein vorangestelltes Info-Symbol. In „Zusammenhänge“ stehen Erweiterungen und Verknüpfungen wie die Gefährdungen als Aufzählung. Alle aufklappbaren Erklärungen, also Blocklegenden und Vokabelkarten, folgen demselben Schema: eine eigene Karte, je Eintrag zuerst „Merkmal: Wert“ (etwa „Modalverb: SOLLTE“, der Wert verlinkt den Vokabulareintrag), darunter die Erklärung; Zusatzspalten folgen als „Merkmal: Wert“-Zeilen. Eine aufgeklappte Karte steht 6 px unter der Zeile ihres Auslösers und hält 12 px zum nächsten Inhalt; enge Stapel wie Listenzeilen und der Pfad über dem Titel setzen das über `tightStackClass` bzw. `beforeTightContentClass` durch (`src/components/legendStyles.ts`, Merkmalsnamen in `getVocabularyTermLabel`, `src/features/vocabularies/vocabularyTitle.ts`).
