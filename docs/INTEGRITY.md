...
- **Catalog-Sync-Guard** (`scripts/catalog-sync-guard.mjs`) prüft bei Sync-PRs Branch und Titel, einen exakt auf `upstream-manifest.json` begrenzten Diff, das strikte Manifest-v2-Schema für das bisherige und das neue Manifest, Registry-Metadaten, kanonische Dateireihenfolge und Signatur. Der neue BSI-Snapshot muss per GitHub Compare API ausnahmslos `ahead` sein; `scripts/sync-upstream-manifest.mjs` blockiert eine Signaturänderung bei unverändertem Snapshot bereits vor jedem Schreibzugriff. Git-Blob-SHAs und Content-Hashes werden gegen den vollständigen Snapshot gebunden; die Namespace-Inventur muss exakt allen direkten `.csv`-Mitgliedern des registrierten Verzeichnisses entsprechen, während externe oder anderweitig unzulässige `ns`-Referenzen weiterhin blockieren. Ein reviewpflichtiger Issue-PR darf einen neuen Snapshot und dadurch notwendige `blocked-by-upstream`-Wechsel atomar einführen, wenn Manifest und Quellregister gemeinsam geändert werden, die Manifest-Pfadmenge sowie `artifactKey` und Root-Typ je Pfad unverändert bleiben und jeder Lifecycle-Wechsel ausschließlich in die Sperre führt. Dieser Pfad validiert das bisherige Manifest und dessen offizielles Repository, das vollständige Quellregister einschließlich BSI-Issue-Pflicht, das neue signierte Manifest gegen das Register, den ausschließlich vorwärts gerichteten Snapshot-Vergleich und sämtliche Dateien gegen den offiziellen BSI-Tree. Keine Prüfung des autonomen Sync-Pfads entfällt oder wird gelockert. API- und Netzwerkfehler sowie `identical`, `behind` und `diverged` blockieren.
